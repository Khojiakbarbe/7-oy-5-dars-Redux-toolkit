...
import { edit } from './features/appSlice'

function EditModal(current) {
    const dispatch = useDispatch()
    const [modal, setModal] = useState(false)

    const [english, setEnglish] = useState(current.current.eng)
    const [uzbek, setUzbek] = useState(current.current.uz)
    const [level, setLevel] = useState(current.current.level)
    ...
    function submit(e) {
        e.preventDefault();
        dispatch(edit({ id: current.current.id, eng: english, uz: uzbek, level, completed }))
        setEnglish('')
        setUzbek('')
        setModal(false)
    }
    ...
    return (
        <div>
            ...
            <button data-modal-target="edit-modal" data-modal-toggle="edit-modal" onClick={() => setModal(true)} className="border text-black hover:bg-gray-400 hover:transition focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm  p-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                ✍
            </button>

            <div id="edit-modal" onClick={(e) => closeModalF(e)} tabIndex="-1" aria-hidden="true" className={`fixed top-0  left-0 right-0 bg-[rgba(143,142,142,0.553);] z-50 ${!modal && 'hidden'} w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full`}>
                <div className="relativ m-[0_auto] items-center w-full max-w-md max-h-full">
                    <div onClick={() => setModal(true)} className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <button onClick={() => setModal(false)} type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="edit-modal">
                            <svg  onClick={() => setModal(false)} className="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fillRule="none" viewBox="0 0 14 14">
                                <path  onClick={() => setModal(false)} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                            </svg>
                            <span className="sr-only">Yopish</span>
                        </button>
                        <div className="px-6 py-6 lg:px-8">
                            <h3 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">O'zgartirish</h3>
